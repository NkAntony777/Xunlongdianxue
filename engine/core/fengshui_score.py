...
def find_and_rank_candidates(
    ...
    *,
    ...
    cands = search_candidates(
        ...
        max_candidates=120, step=4, water=water, ban_buffer_m=_ban_m,
        qi_grid=qi_grid, qi_min_percentile=45.0,
    )
    if len(cands) < 3:
        cands = search_candidates(
            dem, flow_acc=flow_acc_arr, tpi_radius_m=80, tpi_threshold=0.0,
            max_candidates=150, step=3, water=water, ban_buffer_m=_ban_m,
            qi_grid=qi_grid, qi_min_percentile=30.0,
        )
    if len(cands) < 2:
        cands = search_candidates(
            dem, flow_acc=flow_acc_arr, tpi_radius_m=60, tpi_threshold=0.0,
            max_candidates=150, step=2, water=water, ban_buffer_m=_ban_m,
            qi_grid=qi_grid, qi_min_percentile=15.0,
        )
    ...
    def _make_cand_at(pr: int, pc: int, form_boost: float = 0.0) -> AcupointCandidate | None:
        from engine.core.acupoint import classify_form, score_form, AcupointCandidate as _AC
        ...
        ft = classify_form(tpi_p, ls)
        px, py = dem.xy(pr, pc)
        ac = _AC(
            row=pr, col=pc, x=px, y=py,
            elevation=float(dem.data[pr, pc]),
            tpi=tpi_p, twi=0.0,
            form_type=ft,
            form_score=int(round(max(score_form(tpi_p, ft), form_boost))),
            local_slope=ls,
        )
        if _is_on_water(ac):
            return None
    ...
    if primary is not None:
        ...
        if ent_cand is not None and not any(
            c.row == ent_cand.row and c.col == ent_cand.col for c in cands
        ):
            cands = [ent_cand] + list(cands)
        if qi_grid is not None:
            # 入首 600m 窗内局部 qi 峰
            ...
            if sub.size and np.isfinite(sub).any():
                ...
                li = int(np.argmax(filled))
                lr, lc = np.unravel_index(li, filled.shape)
                pr, pc = int(r0 + lr), int(c0 + lc)
                loc = _make_cand_at(pr, pc, form_boost=float(filled[lr, lc]))
                if loc is not None and not any(
                    c.row == loc.row and c.col == loc.col for c in cands
                ):
                    cands = [loc] + list(cands)

    # 热峰强制注入（理论最优必须进池）
    peak_cand = None
    if peak_rc is not None:
        pr, pc, psc = peak_rc
        peak_cand = _make_cand_at(pr, pc, form_boost=float(psc))
        if peak_cand is not None and not any(
            c.row == peak_cand.row and c.col == peak_cand.col for c in cands
        ):
            cands = [peak_cand] + list(cands)
    ...
    def _fuse_overall(form_sc: float, qv: float, d_align: float) -> int:
        ...
        if qv >= qi_p95:
            ...
            da = max(da, 55.0)
        return clamp_score(0.35 * form_sc + 0.50 * qv + 0.15 * da)
    ...
    if peak_fused is not None:
        ...
        if not in_list:
            # 插入并保证至少第 2（若 qi 最高可提到第 1 同分竞争）
            results = [r for r in results if not (
                abs(r.x - peak_fused.x) < 1e-6 and abs(r.y - peak_fused.y) < 1e-6
            )]
            # 高 qi 保送后 overall 应已不低；若仍偏低再抬一档
            if peak_fused.overall < 70:
                peak_fused.overall = clamp_score(max(peak_fused.overall, 72))
